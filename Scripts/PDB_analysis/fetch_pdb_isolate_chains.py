...
output_dirs = [
    "Output/PDBs",
    "Output/PDBs/published_structure",
    "Output/PDBs/asymetric_unit",
    "Output/PDBs/unique_chains"
]
...
for pdb in pdb_names:
    ...
    output_path = f"Output/PDBs/published_structure/{pdb}.pdb"
   ...
   # Initialize a new column 'fibril' with NaN
    ...
    # Initialize lists to store the current chain details
    ...
    # Replace NaNs with a default value (like 1 if that makes sense)
    current_pdb_df['polymorph'].fillna(0, inplace=True)
    ...
    if len(selected_chains) > 1:
        for i, chain in enumerate(selected_chains, 1):
            save_selected_chains([chain], f'Output/PDBs/unique_chains/{pdb}_{i}.pdb')
    else:
        save_selected_chains(selected_chains, f'Output/PDBs/unique_chains/{pdb}.pdb')
    ...
    save_selected_chains(asym_unit_chains, f'Output/PDBs/asymetric_unit/{pdb}_asym_unit.pdb')
...
### SAVING DATA ###
###################
com_and_fibril_df.to_csv('Output/PDBs/COM_and_fibril.csv', index = False)
com_distances_df.to_csv('Output/PDBs/COM_distances.csv', index = False)
min_distance_df.to_csv('Output/PDBs/min_COM_distances.csv', index = False)
