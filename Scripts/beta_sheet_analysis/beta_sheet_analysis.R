...
if(!dir.exists("Output/b_sheets")) dir.create("Output/b_sheets")
if(!dir.exists("Output/b_sheets/ramachandran_plots")) dir.create("Output/b_sheets/ramachandran_plots")
...
#beta_sheet_data <- data.frame()
proportion_data <- data.frame()

# Getting a list of PDB names
#pdb_names <- read.delim("Output/pdb_names.txt")

# Importing Q-score validation data to use only high resolution PDBs
high_resolution <-  read_delim("Output/Validation/high_resolution_pdb_ids.txt", 
                               delim = "\t", escape_double = FALSE, 
                               trim_ws = TRUE)

# Dealing with intra-PDB variation underscores and duplicates
high_resolution$pdb_id <- sub("_.*", "", high_resolution$pdb_id)
high_resolution <- high_resolution %>% distinct(pdb_id, .keep_all = TRUE)


for (pdb_name in high_resolution$pdb_id) {
  
  cat("Analysing -", pdb_name, "\n")  
  ...
  if (!is.null(pdb$atom) && nrow(pdb$atom) > 0) {
    ...
    ggplot(torsion_df, aes(x = phi, y = psi)) +
      ...
      labs(title = paste0("Ramachandran Plot - ", pdb_name),
           x = "Phi (φ) Angles", y = "Psi (ψ) Angles")

    # Saving plots
    ggsave(plot = last_plot(), file = paste0("Output/b_sheets/ramachandran_plots/", pdb_name, ".png"),
           height = 8, width = 8)
    ...
    # Keeping only unique positions to avoid different pdbs having different number of chains
    #b_strands_unique <- b_strands[!duplicated(b_strands$pos), ]
    
    # Merging current data with combined data frame
    #beta_sheet_data <- rbind(beta_sheet_data, b_strands_unique) 
    ...
    cat("Skipping ", pdb_name, ": PDB file is empty or contains no atomic coordinates.")
  }

}

# Removing NAs and UNK residues
#all_torsion_data <- all_torsion_data %>% filter(residue != "UNK")
#all_torsion_data <- all_torsion_data %>% filter(!(is.na(psi) & is.na(phi)))
#beta_sheet_data <- beta_sheet_data %>% filter(!(is.na(pos)))


# Saving data
write.csv(all_torsion_data, file = "Output/b_sheets/all_torsion_data.csv", row.names = FALSE)
#write.csv(beta_sheet_data, file = "Output/b_sheets/beta_sheet_data.csv", row.names = FALSE)
write.csv(proportion_data, file = "Output/b_sheets/proportion_data.csv", row.names = FALSE)
...
#four_plus <- beta_sheet_data[find_consecutive(beta_sheet_data$pos), ]
proportion_data_four_plus <- proportion_data[find_consecutive(proportion_data$pos), ]

# Saving data
#write.csv(four_plus, file = "Output/b_sheets/beta_sheet_data_four_plus.csv", row.names = FALSE)
write.csv(proportion_data_four_plus, file = "Output/b_sheets/proportion_data_four_plus.csv", row.names = FALSE)

#######################################################
### Importing data to avoid running loop every time ###
#######################################################

all_torsion_data <- read.csv("Output/b_sheets/all_torsion_data.csv")
#beta_sheet_data <- read.csv("Output/b_sheets/beta_sheet_data.csv")
#four_plus <- read.csv("Output/b_sheets/beta_sheet_data_four_plus.csv")
proportion_data <- read.csv("Output/b_sheets/proportion_data.csv")
proportion_data_four_plus <- read.csv("Output/b_sheets/proportion_data_four_plus.csv")
...
ggsave(plot = last_plot(), filename = "Output/b_sheets/all_pdbs_ramachandran.png",
       height = 8, width = 8)

# Counting the number of B-sheet occurrences for each residue 
# beta_sheet_counts <- beta_sheet_data %>% count(pos)
# names(beta_sheet_counts)[names(beta_sheet_counts) == "n"] <- "b_sheet"
# beta_sheet_counts <- na.omit(beta_sheet_counts)
# 
# four_plus_counts <- four_plus %>% count(pos)
# names(four_plus_counts)[names(four_plus_counts) == "n"] <- "long_b_sheet"

### Adding in total number of counts for each residue ###
...
# Calculating the percentage occurance of each residue in a B-sheet
# count_df <- merge(beta_sheet_counts, total_counts, by = "pos")
# count_df <- count_df %>% mutate(percentage = (b_sheet / total) * 100)
# 
# count_df <- merge(count_df, four_plus_counts, by = "pos")
# count_df <- count_df %>% mutate(four_plus_percentage = (long_b_sheet / total) * 100)

#################################################################
### Calculating the total b-strand proportion across all pdbs ###
#################################################################

total_PDBs <- as.numeric(nrow(high_resolution))
...
#plot_data <- merge(total_b_strand, total_b_strand_four_plus, by = "pos")

# Saving plot_data
write.csv(plot_data, "Output/b_sheets/plot_data.csv")
...
ggplot(data = plot_data) +
  ...
  xlab("Residue Number") +
  #scale_x_continuous(limits = c(0, 140), breaks = seq(0,999,20)) +
...
ggsave(plot = last_plot(), filename = "Output/b_sheets/strands.png",
       height = 4, width = 10)
...
ggsave(plot = last_plot(), filename = paste0("Output/b_sheets/strands_sliding_window_", window_size, ".png"),
       height = 4, width = 10)
...
ggplot() +
  ...
  geom_line(data = plot_data, aes(x = pos, y = percentage_four_plus), size = 1) + 
...
ggsave(plot = last_plot(), filename = "Output/b_sheets/strands_vs_regions.png",
       height = 4, width = 8)
...
ggplot() +
  geom_rect(data = stable_regions, aes(xmin = min, xmax = max, 
                                       ymin = -Inf, ymax = Inf, 
                                       fill = region), alpha = 0.75) +
  geom_line(data = plot_data, aes(x = pos, y = sliding_window_four_plus), size = 1) + 
  #scale_x_continuous(limits = c(0, 140), breaks = seq(0,140,20)) +
  scale_x_continuous(breaks = seq(0,9999,20)) +
  scale_fill_manual(values = region_colours) +
...
ggsave(plot = last_plot(), filename = paste0("Output/b_sheets/strands_vs_regions_", window_size, ".png"),
       height = 4, width = 8)
...
foldx_stability <- read_csv("Output/thermodynamics/foldx_stability.csv")
...
ggplot() +
  ...
  scale_x_continuous(breaks = seq(0,9999,20)) +
  scale_y_continuous(
    name = bquote(bold("Fraction in " * beta * "-conformation")),
    sec.axis = sec_axis(~ ., name = bquote(bold("Scaled Mean " * Delta * "G"*degree * " (kcal.mol"^-1 * ")")))) +
  scale_fill_manual(values = region_colours, guide = "none") +
  scale_linetype_manual(values = c("B-Strands" = "dashed", 
                                   "DeltaG" = "solid"),
                        name = "Data",
                        label = c(#bquote(bold(beta * "-Strands   ")),
                                bquote(bold(L[beta] * " \u2265 " * 4)),
...
ggsave(plot = last_plot(), filename = paste0("Output/b_sheets/strands_vs_regions_vs_thermo_", window_size, ".png"),
       height = 4, width = 10)
