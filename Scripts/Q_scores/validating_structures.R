...
# Script Logic:
  ...
  # Optional -- remove PDBs with resolution > 4 prior to threshold calculation
...
save_path <- paste0(folder_path, "/plots")
single_pdb_path <- paste0(save_path, "/single_pdbs")
...
for (file in xml_files) {
  ...
  if ("Q_score" %in% all_attributes) {
    ...
    # Merging to main df
    #validation_data <- rbind(validation_data, df)
...
# Importing data
pdb_info <- read.csv("Output/PDBs/COM_and_fibril.csv")
...
write.csv(q_score_df, paste0(folder_path, "/Q_Scores.csv"), row.names = FALSE)
...
# Merging with q_score_df
q_score_df <- merge(q_score_df, resolution_df, by = "PDB")

# Removing non-cryoEM and those with poor resolution
...
write.csv(PDB_q_score, file = paste0(folder_path, "/mean_PDB_Q_score.csv"), row.names = FALSE)
...
ggsave(file = paste0(save_path, "/mean_Q_score.png"), plot = last_plot(), height = 4, width = 8, bg = "white")
...
for (i in pdb_names) {
  ...
  ggsave(file = paste0(single_pdb_path, "/", i, "_Q_score.png"), plot = last_plot(), height = 4, width = 8, bg = "white")
...
write.table(high_resolution_PDBs_df, file = "Output/Validation/high_resolution_pdb_ids.txt", 
...
ggsave(file = paste0(save_path, "/q_score_density.png"), plot = last_plot(), height = 4, width = 6, bg = "white")
...
ggsave(file = paste0(save_path, "/residue_count_stacked_bar.png"), plot = last_plot(), height = 4, width = 8, bg = "white")
...
# Saving final data
write.csv(high_resolution_residues, file = paste0(folder_path, "/high_resolution_residues.csv"), row.names = FALSE)
