...
df = pd.read_csv(os.path.join("Output", "RMSD", "data", "RMSD_cluster_groups.csv"))

# Directory containing PNGs
png_dir = os.path.join("Output", "PNG", "stable_regions", "unique_chains")
...
images_per_row = round(math.sqrt(len(images))) + 2
...
### Asymmetric Unit Figure Creation ###

# Directory containing PNGs
png_dir = os.path.join("Output", "PNG", "stable_regions", "asym_unit")
...
# Create a new blank image with the appropriate size
combined_image = Image.new("RGBA", (total_width, total_height), (255, 255, 255, 0))  # Use RGBA for transparency
...
# Save the combined image
combined_image.save(os.path.join(png_dir, "combined_grid.png"))